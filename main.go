...
func main() {
	localHARFilePath := "www.buckeyeinternational.com.har"
	jsonInput := readAFileAsString(localHARFilePath) // Read JSON input from file

	urls := getJustURLSFromGivenJSON(jsonInput)
	...
	for _, url := range urls {
		url = "https://www.buckeyeinternational.com/" + url // Prepend base URL to each URL
		downloadPDF(url, outputFolder)                      // Download each PDF
	}
}
...
// getJustURLSFromGivenJSON extracts and returns all the URLs from the provided JSON data.
func getJustURLSFromGivenJSON(jsonData string) []string {
	type Media struct {
		Url string `json:"Url"`
	}
	type SdsSheet struct {
		Media []Media `json:"Media"`
	}
	type Data struct {
		SdsSheets []SdsSheet `json:"SdsSheets"`
	}
	type Wrapper struct {
		Data Data `json:"data"`
	}
	...
	var urls []string
	for _, sheet := range result.Data.SdsSheets {
		for _, media := range sheet.Media {
